...
import 'dart:io'
    as i; //"i" also can be another char or word, is just an example
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> _addPathToDatabase(String text) async {
      try {
        ...
        setState(() {
          _eventId =
              loggedInUser.email + _date.toIso8601String() + _time.toString();
        });
        // Add location and url to database
        await Firestore.instance.collection('storage').document().setData(
            {'eventId': _eventId, 'url': imageString, 'location': text});
      } catch (e) {
        print(e.message);
        showDialog(
            context: context,
    ...
    Future<void> _uploadImageToFirebase(PickedFile image) async {
      try {
        ...
        final StorageUploadTask uploadTask =
            firebaseStorageRef.putFile(i.File(image.path));
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(12.0),
//        itemExtent: 106.0,
          children: <Widget>[
            SizedBox(
              height: 30,
